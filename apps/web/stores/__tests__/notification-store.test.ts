...
vi.mock('@/lib/api', () => ({
  api: {
    get: vi.fn(),
    patch: vi.fn(),
    // The store POSTs, not PATCHes. A mock with only `patch` failed both
    // read tests with "api.post is not a function" in FreeFrame for months.
    post: vi.fn(),
  },
}))
...
describe('Notification store', () => {
  ...
  describe('the badge clears before the round trip, not after', () => {
