 ...
 * `fb_setup_done` was absent it fetched /setup/status and, in the
...
describe('the setup branch does not answer the auth question', () => {
  it('redirects an unauthenticated request that has no setup cookie', async () => {
    ...
    const res = await middleware(request('/settings/design'))
  ...
  it('still asks the API when the cookie is missing', async () => {
    await middleware(request('/settings/design'))
  ...
  it('does not ask the API when the cookie is present', async () => {
    await middleware(request('/settings/design', 'fb_setup_done=1'))
...
describe('what the setup branch is actually for, unchanged', () => {
  ...
  it('redirects to /setup even for an authenticated caller', async () => {
    ...
    const res = await middleware(request('/settings/design', 'fb_access_token=abc'))
  ...
  it('does not re-set the cookie when it was already present', async () => {
    const res = await middleware(
      request('/settings/design', 'fb_setup_done=1; fb_access_token=abc'),
    )

    expect(res.cookies.get('fb_setup_done')).toBeUndefined()
  })
  ...
  it('accepts a refresh token alone, as it always has', async () => {
    const res = await middleware(request('/settings/design', 'fb_refresh_token=xyz'))
...
describe('public routes are untouched', () => {
  // No '/share/': share links went with FreeFrame's media features, and
  // middleware.test.ts asserts that the exemption did NOT outlive the
  // route. A public prefix matching nothing is how a hole gets re-opened
  // the day someone adds a page under that name.
  it.each(['/login', '/setup', '/invite/abc123'])(
