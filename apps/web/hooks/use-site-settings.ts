 ...
 *  a mismatched literal there would silently seed nothing. */
...
export function useSiteSettings() {
  ...
  /** The IANA zone scheduled jobs read their wall clock from. Celery itself
   *  stays on UTC; the jobs tick and compare against this, so a change takes
   *  effect on the next tick with no restart. The server rejects an unknown
   *  zone with a 400. */
   ...
   *
   *  Superadmin-only in practice — PATCH /site-settings 403s for anyone
   *  else. Turning it ON does not lock
   ...
   *
   * Deliberately does not touch the four `*_s3_key` columns. A brand image,
   * once set, is never cleared: the backend ignores a null for those columns
   * (see BRAND_IMAGE_FIELDS in routers/site_settings.py), so sending them
   * would be a request that quietly did nothing. Not sending them is what
   * makes this function's name honest about its own scope. There is no
   * `removeLogo`/`removeFavicon` here for the same reason — a clear-to-empty
   * that cannot happen should not be callable.
   */
  ...
  return {
    isLoading,
    orgName: data?.org_name ?? 'FilmBill',
