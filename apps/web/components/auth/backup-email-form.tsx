...
/**
 * Set and confirm the password-reset address.
 *
    ...
    <div className="space-y-4">
      {state === 'pending' && !editing ? null : (
        <form onSubmit={submitAddress} className="space-y-3">
          <Input
            ...
            disabled={busy}
          />
          {/* FreeFrame §201 — body size, not the smallest caption step.
      ...
      {sameDomain && (
        <div className="flex items-start gap-2 rounded-lg border border-status-warning/40 bg-status-warning/10 p-3">
          {/* FreeFrame §201 — this is the one that has to be noticed, so it reads at
