 ...
 *
 * the profile page needed four of these: the emailed code that
 ...
 * regenerating backup codes and starting a replacement enrolment.
 ...
 *
 * The digits come from the shared CodeInput, so this is not a fifth
 ...
 *
 * two additions, both because this dialog is every re-auth in the app:
 *
 *   * a backup code can now actually be typed here. The description has said
 *     "a backup code works too" since FreeFrame §197 while the only field in it threw
...
export function CodeConfirmDialog({
  ...
  /** FreeFrame §205 — "We sent a code to …", for an email-factor user. */
  notice?: string
  /** FreeFrame §205 — rate-limited resend. Absent for a TOTP user, who has no mail to
  ...
  // attempt's digits or its error — nor, since FreeFrame §205, be stuck in the backup
  ...
  async function submit(value: string) {
    // the minimum differs per field. A backup code is nine characters
