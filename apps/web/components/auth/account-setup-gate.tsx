...
/**
 * The blocking onboarding screen.
 *
 ...
 * and a flag is exactly what FreeFrame §200 refused: it would drift from the data,
...
export function AccountSetupGate({ user }: { user: User }) {
  ...
  // see lib/password-policy.passwordSubmitBlock. Shared with the three
  ...
  async function submitPassword(e: React.FormEvent) {
    ...
    try {
      ...
      // rather than unpacked blind, per FreeFrame §196's rule about never storing
        ...
        <section className="rounded-lg border border-border bg-bg-secondary p-5">
          ...
          {needsPassword ? (
            <form onSubmit={submitPassword} className="mt-4 space-y-4">
              ...
              {/* FreeFrame §202 — a blocked submit always says why. Never disabled by a
