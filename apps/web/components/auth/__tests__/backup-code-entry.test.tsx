/**
 * Backup codes can be typed, and only where they are legitimate.
 *
...
// the 2FA-off policy now rides on /auth/me as `two_factor_required`,
...
// and deliberately left alone by FreeFrame §206 — without this mock the form never
...
describe('the backup-code field', () => {
  ...
  it('does NOT strip letters or the dash — the whole bug', async () => {
    ...
    // Before FreeFrame §205 this field did not exist, and the only one that did would
...
describe('re-authenticating as an email-factor user', () => {
  ...
  it('still opens the dialog when the mail cannot be sent', async () => {
    // A backup code is a perfectly good way through this dialog, and FreeFrame §205
