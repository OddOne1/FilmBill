 ...
 *
 * `'2fa-code'`  — enrolled already, completing a login.
...
export function LoginForm() {
   ...
   *
   * With `require_2fa` on, magic-code sign-in is refused by the API,
  ...
  // ─── One place that reads a login response (the same lesson, client-side) ──
   ...
   * that silently produced `setTokens(undefined, undefined)` before FreeFrame.
   */
  ...
  if (activeStep === '2fa-setup') {
    if (setupStage === 'backup') {
      // Shared with the settings page's own enrolment — the codes are
    ...
    return (
      <div className="animate-slide-up">
        ...
        {/* No way back to the magic-code screen on an instance that requires
            ...
            reset there, so the link would lead to a button that
            always fails. */}
        {!requireTwoFactor && (
          <div className="mt-6 text-center">
            <button
  ...
  return (
    <div className="animate-slide-up">
      <div className="mb-8">
        <h1 className="text-xl font-semibold text-text-primary mb-1">Sign in to FilmBill</h1>
        <p className="text-sm text-text-secondary">
