/**
 * The module shape the password meter loads, and the rule that guards it.
 *
 * This file exists because FreeFrame §200 shipped a bug that every test in this repo was
...
describe('buildZxcvbnOptions against the REAL modules', () => {
  it('produces a populated dictionary from namespaces that have no default', async () => {
    // This is the test the FreeFrame §202 bug needed and did not have. The four lines it
...
describe('resolveModuleExports', () => {
  ...
  it('throws a message naming the keys and what it actually got', () => {
    ...
    // read properties of undefined", which is exactly how long FreeFrame §202 took to
    // find.
