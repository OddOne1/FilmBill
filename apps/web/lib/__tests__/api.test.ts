...
// NOTE on the response mocks below: the client reads the body with
// `response.text()` and JSON.parses it itself (see lib/api.ts), so a mock
// that supplies only `json` makes every success path throw
// "response.text is not a function". Every mock here therefore provides
// `text`; `json` is kept for the error paths, which do use it.
