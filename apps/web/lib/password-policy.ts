 ...
 * never does.
 *
 * It matches the server's constants as of FreeFrame §200. Stating it as a fallback
 ...
 * the FreeFrame §202 bug lived in exactly these four lines, in code that could not be
 ...
 * copies. Loudly, because the alternative is what FreeFrame §202 was: a `catch` that
...
/**
 * Why the submit button cannot be pressed yet — or `null` when it can.
 *
 * ONE rule, shared by the four forms that set a password (invite acceptance,
 * the FreeFrame §200 onboarding gate, the login screen's set-password step, and
 ...
 * and the screen says nothing — that is the bug class FreeFrame §202 is about, not the
    ...
    const timer = setTimeout(() => {
      scorePassword(password, inputsKey ? inputsKey.split('\u0000') : [], policy)
        ...
        .catch((err) => {
          ...
          //
          // but it is no longer SILENT. This catch is what hid a
