 ...
 *
 * FreeFrame: there is deliberately no Remove. A brand image that has been set is
 * never cleared back to the FilmBill default (the backend ignores a null
 ...
 *
 * There is no third state, `removed`. A brand image that has been set
 * is never cleared back to the bundled FilmBill default — the backend
...
export default function BrandingPage() {
  ...
  /** Back out of an unsaved pick — NOT a clear. The slot returns to
  ...
  /** Stage "back to defaults" rather than doing it. FreeFrame: a reset should be
   *  as reversible as any other edit until Save.
   *
   *  this no longer touches the logos or favicon. It never could
  ...
  //
  // the logo/favicon conditions are gone from this test along with
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Found by a test; the staleness predates it.
      setDraftLogos((prev) => { Object.values(prev).forEach(revoke); return { dark: KEEP, light: KEEP, login: KEEP } })
      setDraftFavicon((prev) => { revoke(prev); return KEEP })
    } catch (err) {
      savingRef.current = false
  ...
  // reset: a custom name or palette. A logo is deliberately not part
  ...
  const previewName = draftName.trim() || 'FilmBill'
  ...
  return (
    <div className="p-6 max-w-2xl space-y-8 pb-24">
      ...
      <section className="space-y-3">
        ...
        <div className="p-4 rounded-lg border border-border bg-bg-secondary space-y-3">
          ...
          <p className="text-xs text-text-tertiary">
            Shown in the sidebar for everyone in this workspace. Defaults to &ldquo;FilmBill&rdquo;.
          </p>
        </div>
      </section>
      ...
      <section className="space-y-3">
        ...
        {/* stated, not left to be discovered by a Remove button that
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-text-tertiary -mt-1">
          Shown above the sign-in form, before anyone is logged in. Falls back to the default FilmBill logo
          until one is set.
        </p>
        <LogoUploadSlot
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-text-tertiary -mt-1">
          ...
          custom colors keeps FilmBill&apos;s original palette.
        </p>
        ...
        <div className="rounded-lg border border-border bg-bg-secondary p-4 flex items-center gap-2.5">
          <div className="h-7 w-7 rounded-md overflow-hidden flex items-center justify-center bg-bg-tertiary shrink-0">
            {activeLogo ? (
              ...
              <>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/logo-icon.png" alt="FilmBill" className="h-6 w-6 object-contain logo-dark" />
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/logo-icon-dark.png" alt="FilmBill" className="h-6 w-6 object-contain logo-light" />
              </>
            )}
          </div>
