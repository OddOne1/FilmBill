/**
 * Branding settings stage changes instead of applying them (CLAUDE.md FreeFrame).
 *
...
describe('branding draft state', () => {
  ...
  it('Discard reverts the form and sends nothing', async () => {
    ...
    expect(nameField().value).toBe('FilmBill')
    expect(patch).not.toHaveBeenCalled()
    expect(upload).not.toHaveBeenCalled()
  })
  ...
  it('a colour change stages, and Save sends that theme only', async () => {
    ...
    await user.clear(nameField())
    await user.type(nameField(), 'FilmBill')
  ...
  it('a staged full reset saves as one call, not five', async () => {
    ...
    // the logo nulls this used to carry are gone. A brand image that
  ...
  it('a reset that is then edited still sends no logo fields', async () => {
    // this used to assert the opposite: that a reset-then-edit took
