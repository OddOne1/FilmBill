/**
 * Changing your password when you have 2FA on.
 *
 * This is the bug the whole FreeFrame investigation started from, and it is the
 * same one FreeFrame fixed on the login screen, on the one surface FreeFrame
 ...
 *
 * FreeFrame §200 changed the SHAPE of this flow, not its guarantee. Changing a password
 ...
 * endpoint that actually needs it. The FreeFrame §197 guarantee these tests exist for is
...
async function requestPasswordChange(user: ReturnType<typeof userEvent.setup>) {
  ...
  // the button is disabled until the live meter is satisfied, and the
...
/** FreeFrame §200 — the old "hunter2hunter2" fails the policy (no upper case, no
...
describe('password change without 2FA', () => {
  it('verifies the emailed code and sets the password, as before', async () => {
    ...
    vi.mocked(api.post)
      ...
      .mockResolvedValueOnce(TOKENS)                // set-password (FreeFrame §199 pair)
    render(<ProfilePage />)

    await requestPasswordChange(user)
    await typeCode(user, '123456')

    // the pair now comes from /auth/set-password's OWN response, which
  ...
  it('asks for the reset code out of its own pool', async () => {
    const user = userEvent.setup()
    vi.mocked(api.post)
...
describe('password change with 2FA on', () => {
  ...
  it('completes the change after the second factor', async () => {
    ...
    // ONE call, carrying the second factor. There is no
  ...
  it('leaves the session alone when the second factor is wrong', async () => {
    ...
    // the refusal now comes from /auth/set-password itself, so what
