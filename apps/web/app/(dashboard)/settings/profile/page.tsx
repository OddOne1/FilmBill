...
export default function ProfilePage() {
  ...
  // a 2FA-enrolled user's emailed reset code does not, on its own,
  // produce a session (FreeFrame §193 gates it), so the password change has a second
  // step for them.
  //
  // the pending token this used to carry is gone. The second factor is
  ...
  // does not happen on this path (FreeFrame §199: the primary credential here was a
...
async function handleAvatarCropped(blob: Blob) {
  ...
  async function handlePasswordSave(e: React.FormEvent) {
    e.preventDefault()
    setPasswordError('')
    // the old `length < 8` check is gone, not relaxed. It was the only
   ...
   *  a user who has one.
   *
   *  `reauthCode` is the CHANGE-gate proof. Changing a password that
   *  already exists now needs the current second factor, for the same reason
   *  FreeFrame §192 gave for disabling 2FA: a stolen session must not be able to take
   *  the account.
   *
  ...
  async function finishPasswordChange(reauthCode?: string) {
    ...
    // setting a password bumps token_version, which ends every
    ...
    // FreeFrame §196 rule about not storing `undefined` over working tokens.
  ...
  async function handleConfirmPasswordCode(value?: string) {
    ...
    try {
      // the same rule, on a surface it deliberately left alone: branch on
      ...
      // change.
      //
      // `purpose` names the pool this code was minted into. Reset
      ...
      if (res.requires_2fa) {
        ...
        // to clear. The code goes to /auth/set-password, not to
   ...
   *
   *  FreeFrame §192's authenticated branch of /auth/2fa/send-email-fallback is what
   ...
   *  happens at login, and FreeFrame §199 deliberately suppresses even that one when
  ...
  // the same shared rule the other three password forms use.
  ...
  return (
    <div className="p-6 max-w-xl space-y-8">
        ...
        <form onSubmit={handlePasswordSave} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              // live, as soon as both fields differ, instead of only
          ...
          {/* FreeFrame §202 — a blocked submit always says why, and a score that is
      ...
      {/* FreeFrame §200 — where password resets are delivered. Placed between the
      ...
      {/* Log out lives here now that the sidebar avatar is a plain link
          rather than a menu. Removing the popup should not remove the
          only way out. */}
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-bg-secondary shadow-xl p-6">
            ...
            <div className="mt-4 space-y-2">
              {/* The shared digit boxes, rather than a seventh
      ...
      <Dialog.Root open={pw2faDialogOpen} onOpenChange={setPw2faDialogOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-bg-secondary shadow-xl p-6">
            ...
            <div className="flex items-center justify-end gap-2 mt-5">
              {/* Offered to everyone, not only email-primary users: FreeFrame §191's
