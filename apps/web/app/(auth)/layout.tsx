...
export default async function AuthLayout({
  ...
  // One fetch, two consumers: the login logo, and `require_2fa`.
  //
  // The login form has to know `require_2fa` BEFORE its first paint, because
  // that setting decides whether the default screen is email+password or the
  // magic-code step — and magic-code sign-in 403s on an instance that
  // requires 2FA. Seeding SWR from the value this layout already awaited is
  // the same pattern the dashboard layout uses and costs no extra request;
  // without it the form paints the wrong screen and swaps it out once a
    ...
    <div className="relative min-h-screen bg-bg-primary flex flex-col items-center justify-center px-4">
      ...
      <div className="relative mb-10">
        {loginLogoUrl ? (
          ...
          <img src={loginLogoUrl} alt="FilmBill" className="h-12 w-auto max-w-[220px] object-contain" />
        ) : (
          <Image
            src="/logo-full.png"
            alt="FilmBill"
