"""Let one user past the FreeFrame §200 onboarding gate, from the shell.
...
Run on the server, from the repo root, inside the api container:
    ...
    docker compose -f docker-compose.prod.yml --env-file .env.prod -p filmbill \\
...
def main(email: str, write: bool) -> int:
    ...
    try:
        ...
        # `mathias@` are one account, and the retired half of a merged
