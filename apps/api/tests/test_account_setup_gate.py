"""The two-address split, the onboarding gate, and the escape hatches.
...
bought nothing. FreeFrame §200 splits the two channels rather than patching around
them.
...
class TestTheBackupAddressRules:
    ...
    def test_the_same_address_in_different_case_is_refused(self, client, mock_db):
        """`Mathias@` and `mathias@` are one mailbox (FreeFrame §13a's lesson)."""
        from apps.api.main import app
        from apps.api.middleware.auth import get_current_user
...
class TestTheGateBlocksEveryProtectedRoute:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"cookies": {"fb_account_setup_done": "1"}},
...
class TestTheMigrationShape:
    def test_an_existing_row_is_gated_and_keeps_its_session(self):
        ...
        # the deploy — the property FreeFrame §199's own migration was built around.
