"""Re-auth codes that arrive, and an off switch that respects policy.

Three of the four defects FreeFrame §205 fixes live on the server:
...
arrangement after FreeFrame §200 — had no source for a code at all. All three actions
...
login copy was wrong for enrolment in FreeFrame §203: nobody is signing in.
...
class TestTheReauthCodeIsSent:
    ...
    def test_the_code_lands_in_the_CHALLENGE_pool(self, client, mock_db):
        ...
        FreeFrame §204 it reads the challenge pool and cannot see the enrolment pool at
    ...
    def test_opening_a_dialog_does_not_invalidate_a_live_code(self, client, mock_db):
        """`force=false` by default (FreeFrame §194's rule): a code already in the
...
class TestTheReauthMailCopy:
    """FreeFrame §203's test shape, extended to the new purpose."""
    ...
    def test_it_does_not_tell_the_reader_to_sign_in(self):
        """The FreeFrame §203 mistake, one purpose further on: nobody is signing in
        here either."""
        _subject, html, text = _render("two_factor_reauth")
        body = _visible(html)
...
class TestBackupCodesRemainRedeemable:
    """The browser half of FreeFrame §205 is what was broken; this pins that the server
