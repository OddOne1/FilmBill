"""Password-reset codes live in their own Redis pool.
...
won, silently. FreeFrame had already refused that arrangement between login codes
and 2FA codes ("different operations that should not be able to collide or
overwrite each other's pending code"); in FreeFrame the case for separating
...
exactly the compromised-inbox takeover that was closed on the login door.
"""
import uuid
...
class TestTheTwoPoolsDoNotOverwriteEachOther:
    ...
    def test_every_prefix_in_this_file_is_distinct(self):
        ...
        and the 2FA code. A prefix accidentally reused would recreate the
...
class TestAUsersOwnTwoFactorStateIsReadable:
    """the settings screen needs "is it on, and which method" and had
...
class TestTheTwoFactorGateIsUnchangedForBOTHPurposes:
    """The property FreeFrame is careful NOT to touch.
