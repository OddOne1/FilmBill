"""An enrolment code is not a sign-in code.

FreeFrame §203 put a sentence in the enrolment mail:
...
its whole ten-minute life. The `purpose` argument FreeFrame §203 added changed the
...
Two user-visible symptoms shared the root cause, and FreeFrame §203's copy split is what
...
class FakePool:
    ...
    fake keyed by string prefix would reproduce the very coupling FreeFrame §204
    removes.
    """
...
# ── 1. FreeFrame §203's sentence, as an assertion ─────────────────────────────────────
...
class TestASetupCodeCannotSignYouIn:
    def test_the_login_challenge_rejects_an_enrolment_code(
        ...
        """The headline. Fails before FreeFrame §204."""
...
class TestTheIdempotencyWindowsDoNotCross:
    def test_a_live_challenge_code_no_longer_suppresses_an_enrolment_send(
        ...
        Before FreeFrame §204 the enrolment call found a live code in the shared pool,
    ...
    def test_each_pool_keeps_its_own_idempotency(
        ...
        """Separating them must not lose the FreeFrame §194 rule that made them quiet:
...
class TestTheAttemptsBucketsAreSeparate:
    """Modelled on FreeFrame §197's separated-pools test.
...
class FakeRedis:
    ...
    every one of them. Caught while mutation-checking FreeFrame §204, and this class is
...
class TestTheRealPoolsDoNotShareBuckets:
    ...
    def test_storing_one_does_not_overwrite_the_other(self, fake_redis):
        ...
        state FreeFrame §204's idempotency fix deliberately allows."""
        from apps.api.services import redis_service as rs
    ...
    def test_a_setup_code_does_not_verify_against_the_challenge_pool(
        ...
        """The FreeFrame §204 property at the storage layer, with no router involved."""
        from apps.api.services import redis_service as rs
...
class TestTheKeysThemselvesDiffer:
    def test_every_code_pool_has_its_own_prefixes(self):
        """The FreeFrame §197/FreeFrame §200 invariant, extended.
        ...
        one be redeemed for the other, which is what FreeFrame §204 is about.
        """
        from apps.api.services import redis_service as rs
    ...
    def test_the_enrolment_code_key_is_not_the_staging_key(self):
        ...
        code keyed by email, and FreeFrame §194b's staged secret keyed by user id.
