"""A configured logo or favicon is never cleared back to the default.
...
def test_unrelated_fields_are_untouched_by_the_guard(
    ...
    """Only the four brand-image columns are protected.

    Every other column stays ordinarily writable, in both directions —
    theme_colors back to NULL is the "use the built-in palette" case, and a
    guard written as "ignore nulls on site-settings" would have frozen it.
    """
    configured.theme_colors = {"dark": {"--accent": "#ff0000"}}

    resp = client.patch(
        "/site-settings",
        json={"theme_colors": None},
        headers=auth_headers,
    )
