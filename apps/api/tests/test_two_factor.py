...
def _stage(store, user, secret, method="totp"):
    """Plant what /auth/2fa/setup would have staged.
...
class TestThePendingTokenIsInert:
    ...
    @pytest.mark.parametrize("path", ["/auth/me", "/admin/users", "/me/notifications"])
...
class TestTheSecretIsNotStoredInPlaintext:
    ...
    def test_the_jwt_secret_itself_cannot_read_an_encrypted_secret(self):
        """Domain separation, asserted by outcome rather than by reading the
        derivation.

        The encryption key is derived from the JWT secret through HKDF with
        its own info string, so the obvious naive derivations — the secret
        hashed, or padded straight into a Fernet key — do not open what this
        module wrote. That is what "neither key can be used to attack the
        ...
        token = totp_service.encrypt_secret(totp_service.generate_totp_secret())
        raw = settings.jwt_secret.encode()

        for naive_key in (
            base64.urlsafe_b64encode(hashlib.sha256(raw).digest()),
            base64.urlsafe_b64encode(raw.ljust(32, b"\0")[:32]),
        ):
            with pytest.raises(InvalidToken):
                Fernet(naive_key).decrypt(token.encode())

        # And the module's own key still does open it, so the above is not
        # passing because encryption is broken in general.
        assert totp_service.decrypt_secret(token) is not None
...
class TestEmailFallbackIsSeparateFromMagicCode:
    ...
    def test_it_uses_a_different_rate_limit_bucket(self, client, mock_db):
        ...
        Observed at the limiter itself: what action string each endpoint
        spends is recorded and compared.
        """
        spent = []

        def _record(ip, action, max_requests, window_seconds):
            spent.append(action)
            return (True, 0)
        ...
        with patch("apps.api.middleware.rate_limit.check_rate_limit", side_effect=_record):
    ...
    def test_the_email_says_something_different(self):
        ...
        Both messages are actually rendered and compared, rather than the
        task's source being grepped for the branch that produces them.
        """
        ...
        def _capture(to_email, subject, html_body, text_body=None):
            sent.append((subject, html_body, text_body))
            return True

        with patch.object(email_tasks, "_send_email", side_effect=_capture), \
             patch.object(email_tasks, "render_template", side_effect=lambda name, **ctx: f"<{name}>"):
            email_tasks.send_magic_code_email("u@example.com", "123456", purpose="login")
            email_tasks.send_magic_code_email("u@example.com", "123456", purpose="two_factor")

        (login_subject, _, login_text), (twofa_subject, _, twofa_text) = sent
        assert login_subject != twofa_subject
        assert login_text != twofa_text
        # The 2FA copy is the one that tells the reader something is wrong.
        assert "password" in twofa_text.lower()
        assert "password" not in login_text.lower()
...
class TestSetupAndConfirm:
    def test_setup_stages_a_secret_and_writes_NOTHING_to_the_row(
        ...
        """this used to assert the secret landed on the user. It
    ...
    def test_confirming_without_ever_starting_setup_is_a_400(
        ...
        """"nothing staged" is now what this means, and it covers
