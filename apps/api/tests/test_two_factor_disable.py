"""Disabling 2FA and replacing backup codes.
...
def _enrolled(secret=None, codes=None, email="u@example.com"):
    ...
    # NOT a User field. This suite's mock_db answers every `.first()`
    ...
    u.token_version = 0
    # an enrolled user always has one, and disable has to clear it
...
class TestSelfServiceDisable:
    ...
    def test_it_clears_the_chosen_method_too(self, client, app, mock_db):
        """a stale "totp"/"email" left on a disabled account would
...
class TestAdminOverride:
    ...
    def test_it_clears_the_chosen_method_too(self, client, app, mock_db):
        """the admin path clears exactly what the self-service path
...
class TestEmailFallbackReachableWhenAuthenticated:
    def test_an_authenticated_user_can_request_one_without_a_pending_token(
        ...
        """The self-service paths accept an emailed code as proof, and an
    ...
    def test_the_pending_token_path_still_works_unchanged(self, client, mock_db):
        """The mid-login case must not have been traded away for the new
        one."""
    ...
    def test_verification_is_keyed_by_email_not_by_token(self):
        ...
        Demonstrated by storing a code under one casing and verifying it
        under another, with no token anywhere in the call — rather than by
        grepping the function for "email.lower()", which a comment
        satisfies (CLAUDE.md rule 11).
        """
        ...
        with patch.object(rs, "get_redis", return_value=_FakeRedis()):
            rs.store_2fa_email_code("User@Example.COM", "424242")
            ok, _ = rs.verify_2fa_email_code("user@example.com", "424242")
