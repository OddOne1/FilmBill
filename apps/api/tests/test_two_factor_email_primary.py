"""Email as the PRIMARY second factor, not only a fallback.

FreeFrame shipped email as a way out for someone who had lost their
...
def _stage(store, user, method, secret=None):
    """What /auth/2fa/setup would have staged.
...
class TestLoginMailsTheCodeForAnEmailPrimaryUser:
    def test_the_code_is_sent_without_being_asked_for(self, client, mock_db):
        """The whole difference from the fallback path: no "send me one" click
...
class TestEnrolByEmail:
    ...
    def test_setup_stages_the_choice_and_leaves_the_row_alone(
        ...
        """the choice is staged, not written. An abandoned setup
