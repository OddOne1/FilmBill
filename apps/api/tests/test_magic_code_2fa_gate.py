"""A magic code is a primary credential, not a second factor.

FreeFrame gated /auth/login and left /auth/verify-magic-code alone,
reasoning it was "a separate feature". It is not — it is the other half of
the same login screen ("email + code" by default, "sign in with password
instead" as the fallback link), and it returned full tokens the moment the
Redis code checked out. An enrolled user was therefore fully authenticated
by anyone who could read one email, which defeats 2FA against precisely the
threat it is usually deployed for: a compromised inbox.
...
class TestTheBranchIsActuallyShared:
    ...
    Checked by SUBSTITUTION, not by reading the source. Both endpoints are
    driven with a stand-in `_login_outcome` that returns an unmistakable
    marker; an endpoint that reached the marker called the shared helper,
    and an endpoint that built its own branch could not have. CLAUDE.md
    rule 11 — FreeFrame's version of this grepped the function bodies for
    `_login_outcome(db, user)`, which a comment satisfies.
    """

    _MARKER = "shared-helper-was-called"

    def _stub_outcome(self):
        from apps.api.schemas.auth import TokenResponse

        # **kwargs so this keeps standing in for the real helper as its
        # signature grows (it took a `via` keyword when the emailed second
        # factor was restricted after a magic-code login).
        return lambda db, user, **kwargs: TokenResponse(
            access_token=self._MARKER,
            refresh_token=self._MARKER,
        )

    def test_password_login_goes_through_the_shared_helper(self, client, mock_db):
        user = _user(two_factor_enabled=True)
        mock_db.first.return_value = user
        ...
        assert resp.status_code == 200
        assert resp.json()["access_token"] == self._MARKER

    def test_magic_code_login_goes_through_the_same_helper(self, client, mock_db):
        user = _user(two_factor_enabled=True)
        mock_db.first.return_value = user
    ...
    def test_neither_path_builds_its_own_2fa_branch(self, client, mock_db):
        """The substitution above would still pass if a path ALSO had its own
        copy sitting BEFORE the call — it would answer from the copy and
        never reach the stub.

        So: an ENROLLED user, whose own branch would return requires_2fa, is
        driven through a stub that returns tokens. Anything other than the
        marker means that path decided for itself.
        """
        for endpoint, body, extra in (
            ("/auth/login", {"password": "hunter2hunter2"},
             patch("apps.api.routers.auth.verify_password", return_value=True)),
            ("/auth/verify-magic-code", {"code": "123456"},
             patch(_REDIS_OK, return_value=(True, ""))),
        ):
            user = _user(two_factor_enabled=True)
            mock_db.first.return_value = user
            with patch("apps.api.routers.auth._login_outcome",
                       side_effect=self._stub_outcome()), extra:
                resp = client.post(endpoint, json={"email": user.email, **body})

            payload = resp.json()
            assert payload.get("requires_2fa") is False, endpoint
            assert payload.get("access_token") == self._MARKER, endpoint

    def test_the_helper_is_what_decides_on_the_second_factor(self, client, mock_db):
        """And with the real helper in place, both paths reach the same
        three outcomes for the same user state."""
        for require_2fa, enrolled, expected in (
            (False, True, {"requires_2fa": True, "setup_required": False}),
            (True, False, {"requires_2fa": True, "setup_required": True}),
            (False, False, {"requires_2fa": False}),
        ):
            ...
            for key, value in expected.items():
                assert by_password.get(key) == value, (key, "password", expected)
                assert by_code.get(key) == value, (key, "magic code", expected)

    def test_both_endpoints_declare_the_same_response_model(self):
        """The client is written against one union, not two shapes that
