"""Changing what a session is entitled to actually ends that session.
...
#: every path that sets a password now runs the policy, and
...
#: 500s regardless of auth, so a failure there would say nothing about the
#: token. GET /me/notifications is gated on the same get_current_user and
#: serialises an empty list against the mock session, so a 200 here means
#: the token was accepted and nothing else.
#:
#: FreeFrame probed GET /projects, which left with the media features.
PROTECTED = "/me/notifications"
...
class TestAStaleTokenIsRefused:
    ...
    def test_refresh_does_not_re_run_the_login_gate_on_a_mismatch(
        ...
        branch inside refresh — exactly the duplication FreeFrame §193 removed from the
        login paths and FreeFrame §196/FreeFrame §198 then had to remove again on the client.
