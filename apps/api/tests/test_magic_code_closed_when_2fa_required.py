"""Magic-code sign-in and self-registration close once 2FA is required.

FreeFrame's own history settled what a magic code actually is: a COMPLETE primary
...
class TestNothingChangesWhile2faIsOff:
    ...
    def test_an_unknown_email_still_self_registers(self, client, mock_db):
        ...
        FreeFrame changes it only when `require_2fa` is on."""
...
class TestVerifyMagicCodeNeedsNoGateOfItsOwn:
    def test_a_code_issued_before_the_switch_still_verifies_into_the_2fa_gate(
        ...
        They grant nothing meanwhile: FreeFrame already stops every magic code at
        the second factor."""
