"""What each code mail actually says, and which purpose each caller sends.

Mathias turned two-factor ON from Settings, already signed in, and got a mail
headed **"Your login code — Use this code to sign in to FilmBill."** The
...
template forever and the test would stay green. Same family as FreeFrame §202: the check
...
#: enrolment codes live in their OWN pool now, so the enrolment path
...
#: is deliberately absent: FreeFrame §203 does not touch it, and a test that swept it in
...
class TestEachPurposeRendersItsOwnCopy:
    ...
    def test_enrolment_does_not_tell_the_reader_to_sign_in(self):
        ...
        The mail Mathias received said "Use this code to sign in to FilmBill"
        ...
        assert "Use this code to sign in to FilmBill" not in body
        assert "Use this code to sign in to FilmBill" not in text
    ...
    def test_the_login_mail_is_unchanged_in_substance(self):
        """FreeFrame §203 does not restyle the highest-volume mail this app sends."""
        subject, html, _text = render("login")
        body = visible_text(html)

        assert subject == f"Your FilmBill login code: {CODE}"
        assert "Your login code" in body
        assert "Use this code to sign in to FilmBill" in body
...
class TestAnUnknownPurposeIsNotAnError:
    ...
    def test_the_neutral_wording_takes_no_side_about_signing_in(self):
        ...
        for phrase in ("sign in to FilmBill", "someone has your password",
                       "someone is signed in as you"):
            assert phrase not in body
            assert phrase not in text

    def test_the_legacy_two_factor_label_still_reads_as_a_challenge(self):
        """FreeFrame §191 called this purpose "two_factor". A Celery message queued
...
class TestTheFlowsStillDeliverAWorkingCode:
    ...
    def test_the_idempotency_window_is_unchanged(self, client, mock_db):
        ...
        not mail two codes and invalidate the one being read."""
        user = _user(two_factor_enabled=True, method="email")
        mock_db.first.return_value = user
