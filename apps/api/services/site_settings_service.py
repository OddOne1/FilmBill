"""Reads of the site-settings singleton that several routers need.
...
FreeFrame settled for `timezone`: an admin changing a setting expects it to
...
def instance_org_name(db: Session) -> str:
    ...
    "FilmBill" in its users' authenticators.
    """
    row = _settings(db)
...
def password_required_after(db: Session) -> Optional[datetime]:
    """When passwordless magic-code sign-in closes on this instance.
