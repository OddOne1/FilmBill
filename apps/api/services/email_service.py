...
def email_logo_url() -> Optional[str]:
    ...
    Deliberately NOT `proxy_url_for` (routers/files.py): that returns a
    *relative*, token-authenticated URL that expires after 24 hours. Neither
    ...
    try:
        ...
        logger.warning("Could not resolve email logo: %s", e)
        return None
    finally:
        db.close()
...
class EmailService:
    ...
    def _send_via_ses(
        ...
        try:
            ...
            logger.warning("SES error sending to %s: %s", to_email, e.response["Error"]["Message"])
            return False
    ...
    def _send_via_smtp(
        ...
        try:
            ...
            logger.warning("SMTP error sending to %s: %s", to_email, e)
            return False
    ...
    def send_invite_email(self, to_email: str, inviter_name: str, org_name: str, invite_link: str) -> bool:
        ...
        subject = f"You've been invited to join {org_name} on FilmBill"
        html_body = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            {_logo_img_tag()}
            <h2>You're invited!</h2>
            <p><strong>{inviter_name}</strong> has invited you to join <strong>{org_name}</strong> on FilmBill.</p>
            <p>
                <a href="{invite_link}" 
        ...
        """
        text_body = f"{inviter_name} has invited you to join {org_name} on FilmBill. Click here to accept: {invite_link}"
