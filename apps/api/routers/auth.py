...
def send_magic_code(body: SendMagicCodeRequest, db: Session = Depends(get_db)):
    ...
    both of those stop once 2FA is required instance-wide. A magic
    code is a COMPLETE primary credential, so an instance
    ...
    way back in. A reset still lands in the 2FA gate on the way through,
    ...
    # a reset code goes into its own pool, never the login one. They
    # shared `magic_code:{email}` and an unconditional setex, so asking for
    # one destroyed the other; and in FreeFrame they are not even subject to
...
def verify_magic_code(body: VerifyMagicCodeRequest, db: Session = Depends(get_db)):
    ...
    # checks the pool matching the stated purpose. Trusting the
    ...
    # a correct magic code is a COMPLETE primary credential, not a
    ...
    #
    # via=VIA_MAGIC_CODE: this login proved control of the MAILBOX. That
    # rules the emailed second factor out for the rest of this flow — see
    # _login_outcome and /2fa/send-email-fallback.
...
def get_invite_info(token: str, db: Session = Depends(get_db)):
    ...
    return InviteInfoResponse(
        email=user.email,
        name=user.name,
        # Was left unset, so the invite-accept screen rendered its headline
        # ("You've been invited to join ___") as an empty line. It is the
        # instance's own name, which this endpoint is unauthenticated for
        # the same reason GET /site-settings is: it is what the page needs
...
def login(body: LoginRequest, db: Session = Depends(get_db)):
    ...
    the password check below is UNCHANGED. What changed is what
    ...
    # shared with /auth/verify-magic-code, which is the same login
...
def _login_outcome(db: Session, user: User, *, via: str = VIA_PASSWORD) -> LoginResponse:
    """What a successful primary-credential check leads to.
    ...
    the shape this codebase keeps getting bitten by; FreeFrame carried the same
    ...
    if user.two_factor_enabled:
        ...
        # for an email-primary user the code is sent HERE, without
        ...
        #
        # NOT sent when the primary credential was itself a magic code: that
        # code came out of the same inbox, so mailing a second one there
        # proves nothing new. Such a user completes with their authenticator
        # or a backup code — which is exactly what backup codes are for —
        # or signs in with their password instead.
...
def setup_two_factor(
    ...
    `method` picks which factor is being enrolled. confirm-setup has
    ...
    with the staged setup rather than being re-declared later.

    this endpoint no longer writes to the user row at all. It used
    ...
    if user.two_factor_enabled:
        # replacing a live second factor is the same class of action
        # as removing one: it decides what "a valid second factor" means for
        # this account from here on. FreeFrame already refuses removal on a bearer
...
def confirm_two_factor_setup(
    ...
    "the chosen factor" rather than "the authenticator": for an email
    ...
    regenerating them (a FreeFrame concern), not recovering them.
    """
    user = current_user
    ...
    # the candidate this confirms is the one setup staged, never the
...
def send_two_factor_email_fallback(
    ...
    reachable two ways, like /auth/2fa/setup: mid-login with a
    pending token, and from a SESSION with none.

    The authenticated case is not hypothetical. The self-service disable
    ...
    if user is None:
        ...
        # The mid-login case is the one that can collapse two factors into
        # one. A caller who reached the gate WITH A MAGIC CODE has already
        # proved control of this mailbox; mailing them the second factor
        # there proves nothing further, and whoever can read the inbox holds
        # both halves of the login.
        #
        # Refused explicitly rather than silently dropped: the caller holds
        # a valid pending token for this very account, so there is nothing
        # left to enumerate, and a cheerful 200 would leave a legitimate
        # user watching an inbox that will never fill. They complete with
        # their authenticator or a backup code, or sign in with their
        # password instead — both are named in the message.
...
def disable_two_factor(
    ...
    # the chosen method goes with it. A stale "email" left on a
