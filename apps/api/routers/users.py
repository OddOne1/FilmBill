...
async def upload_avatar(
    ...
    already works (see routers/files.py's module docstring): the bucket
    never needs to be reachable from outside the Docker/LAN network, for
    uploads either.
    """
...
def invite_user(body: InviteRequest, db: Session = Depends(get_db), current_user: User = Depends(require_superuser)):
    ...
    send_task_safe(send_invite_email, user.email, current_user.name or "Admin", instance_org_name(db), invite_url)
...
def delete_user(user_id: uuid.UUID, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    ...
    auth (get_user_by_id), admin, and setup.

    There is no hard-delete endpoint. FreeFrame had one (it existed to
    reclaim a user's stored media); in a billing system a user is referenced
    by every document they issued and every audit row they wrote, so
    removing the row is not a thing the product may offer."""
    user = db.query(User).filter(User.id == user_id, User.deleted_at.is_(None)).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
