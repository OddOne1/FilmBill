...
#: value, it is never cleared back to NULL .
#:
#: NULL means "show the bundled FilmBill default", and the requirement is
...
def get_site_settings(db: Session = Depends(get_db)):
    """Deliberately unauthenticated: it backs the login page's branding, so
    it must answer before anyone has a token.

    That is also why nothing sensitive may ever be added to this response —
    mail credentials live in a separate table behind a separate router for
    exactly this reason (see models/email_settings.py)."""
...
def update_site_settings(
    ...
    # rejected rather than coerced. An unknown zone would make every
    ...
    for field, value in update_data.items():
        # a configured brand image is never cleared. Ignored rather
