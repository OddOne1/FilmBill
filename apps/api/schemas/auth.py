...
class TokenResponse(BaseModel):
    ...
    #: ALWAYS present and always False here, so a caller has exactly
...
#:
#: Email started life in FreeFrame as a fallback for a lost authenticator and is
...
class TwoFactorRequiredResponse(BaseModel):
    ...
    pending_token: str
    #: which code to ask for, so the client can draw the right screen
...
class TwoFactorSetupRequest(BaseModel):
    ...
    #: which second factor to enrol. Defaults to "totp", so every
    #: caller written before this field existed keeps working unchanged.
    method: TwoFactorMethod = "totp"
    #: proof that the caller still holds the CURRENT second factor,
...
class TwoFactorSetupResponse(BaseModel):
    ...
    the TOTP fields are now optional, because an email enrolment has
...
class TwoFactorConfirmResponse(BaseModel):
    ...
    #: what the user ended up enrolled with.
    method: TwoFactorMethod = "totp"
...
class AdminUserResponse(UserResponse):
    """What the superadmin user-management dashboard reads.

    Identical to UserResponse today. It stays a distinct name because P0b
    adds per-company role memberships here (CLAUDE.md rule 6) and that list
    has no business in the response every authenticated caller gets.
    """
...
class VerifyMagicCodeRequest(BaseModel):
    email: EmailStr
    code: str
    #: which pool to check. Login codes and password-reset codes are
